...
class NOTankAiF_VVRP_AlexV3(IStrategy):
    """
        ...
        for i in range(len(df)):
            ...
            if finalH_period == finalL_period:
                for key in MML_LEVEL_NAMES:
                    murrey_levels_data[key][i] = current_close
                continue
            levels = NOTankAiF_VVRP_AlexV3._calculate_mml_core(mn_period, finalH_period, mx_period, finalL_period, mml_c1, mml_c2)
            for key in MML_LEVEL_NAMES:
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float, rate: float,
        ...
        # Block these exit types if not profitable
        conservative_exits = [
            "Strong_VVRP_Short_Zone_Exit", "Strong_VVRP_Long_Zone_Exit",
            "Exit_Min_Check", "Exit_Min_Confirmed",
            "Exit_Max_Check", "Exit_Max_Confirmed",
            "Opposite_Signal_Long_Exit", "Opposite_Signal_Short_Exit"
        ]
        if exit_reason in conservative_exits and current_profit_ratio < 0:
            logger.info(f"{pair} Exit '{exit_reason}' skipped due to negative ROI: {current_profit_ratio:.4f}")
            return False
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        # VVRP Integration
        dataframe = calculate_rolling_vvrp(dataframe, lookback_period=120, num_bins=9, max_bars=200)
        dataframe['VVRP_High_Volume_Long'] = 0
        dataframe['VVRP_High_Volume_Short'] = 0
        for i in range(len(dataframe)):
            for j in range(1, 10):
                mid_price = dataframe.get(f'VVRP_Mid_Price_{j}', pd.Series([None]*len(dataframe))).iloc[i]
                buy_bars = dataframe.get(f'VVRP_Buy_Bars_{j}', pd.Series([0]*len(dataframe))).iloc[i]
                sell_bars = dataframe.get(f'VVRP_Sell_Bars_{j}', pd.Series([0]*len(dataframe))).iloc[i]
                price = dataframe['close'].iloc[i]
                atr = dataframe['atr'].iloc[i] if 'atr' in dataframe else 1.0
                if pd.notnull(mid_price) and abs(price - mid_price) < atr * self.tightness_factor.value:
                    if buy_bars - sell_bars > self.dominance_threshold.value:
                        dataframe.at[i, 'VVRP_High_Volume_Long'] = 1
                    if sell_bars - buy_bars > self.dominance_threshold.value:
       ...
       # Transitional long entry
        ...
        if self.can_short:
            df.loc[
                ...
                (df["close"] < df["[8/8]P"]),       # MML rejection
...
def calculate_rolling_vvrp(dataframe: pd.DataFrame, lookback_period: int, 
    ...
    dataframe['buy_volume'] = np.where(
        ...
        dataframe['volume'] * (dataframe['close'] - dataframe['open']) / dataframe['price_range'].replace(0, np.nan),
        0
    )
    dataframe['sell_volume'] = np.where(
        dataframe['close'] < dataframe['open'],
        dataframe['volume'] * (dataframe['open'] - dataframe['close']) / dataframe['price_range'].replace(0, np.nan),
        0
    )
    dataframe['buy_volume'] = np.where(dataframe['price_range'] == 0, dataframe['volume'] * 0.5, dataframe['buy_volume'])
    dataframe['sell_volume'] = np.where(dataframe['price_range'] == 0, dataframe['volume'] * 0.5, dataframe['sell_volume'])
    dataframe['buy_volume'] = dataframe['buy_volume'].fillna(0)
    dataframe['sell_volume'] = dataframe['sell_volume'].fillna(0)
