...
class TankAiRevival(IStrategy):
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime,
        ...
        SLT0 = current_candle[f'h2_move_mean{pair_tag}']
        SLT1 = current_candle[f'h1_move_mean{pair_tag}']
        SLT2 = current_candle[f'h0_move_mean{pair_tag}']
        SLT3 = current_candle[f'cycle_move_mean{pair_tag}']
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float,
        ...
        if exit_reason == 'Extrema' and trade_duration > self.time4.value:
            logger.info(f"{trade.pair} blocking small extrema!!!")
            # self.dp.send_msg(f'{trade.pair} trailing stop price is below 0')
            return False

        if exit_reason == 'Extrema Full Send' and trade_duration > self.time4.value:
            logger.info(f"{trade.pair} blocking small extrema!!!")
            # self.dp.send_msg(f'{trade.pair} trailing stop price is below 0')
            return False
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[
            ...
            ['enter_long', 'enter_tag']] = (1, 'Extrema')
