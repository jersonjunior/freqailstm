...
class TankAiRevival(IStrategy):
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float,
        ...
        if exit_reason.startswith('Extrema M') and trade_duration > self.time4.value:
            logger.info(f"{trade.pair} blocking small extrema!!!")
            # self.dp.send_msg(f'{trade.pair} trailing stop price is below 0')
            return False

        if exit_reason.endwith('Full Send') and trade_duration > self.time4.value:
            logger.info(f"{trade.pair} blocking small extrema!!!")
            # self.dp.send_msg(f'{trade.pair} trailing stop price is below 0')
            return False
    ...
    def feature_engineering_expand_basic(self, dataframe, metadata, **kwargs):
        ...
        ###########
        dataframe = pd.concat([dataframe, pd.DataFrame(heikinashi)], axis=1)
...
def nadaraya_watson(dataframe, h, r, x_0, smoothColors, lag, mult=2):
    ...
    dataframe = pd.concat([dataframe, pd.DataFrame(df)], axis=1)
    return dataframe

#box_size_percentage = 2  # Example: 2%

def calculate_pnf_with_percentage(df, box_size_percentage, reversal_size):
    ...
    df = pd.concat([df, pd.DataFrame(dataframe)], axis=1)
    return df
