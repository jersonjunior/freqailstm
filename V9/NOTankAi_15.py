...
class NOTankAi_15(IStrategy):
    ...
    * WIP
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        last_natr = 1.0
        # Check if 'generic_tags' column exists and process if so
        if 'generic_tags' in df.columns:
            generic_tags = df.iloc[-1]['generic_tags']
            
            if generic_tags and "last_natr:" in generic_tags:
                # Extract `last_natr` from `generic_tags`
                for part in generic_tags.split(";"):
                    if part.startswith("last_natr:"):
                        last_natr = float(part.split(":")[1])
                        break
    ...
    def set_freqai_targets(self, dataframe, **kwargs):
        ...
        if dataframe.get('extrema_norm') is not None:
            dataframe['&s-extrema'] = dataframe['extrema_norm'].shift(-n)
        else:
            dataframe['&s-extrema'] = 0

        #dataframe['&s-extrema10'] = dataframe['&s-extrema'].rolling(window=10, win_type='gaussian', center=True).mean(std=0.5)
        #dataframe['&s-extrema20'] = dataframe['&s-extrema'].rolling(window=20, win_type='gaussian', center=True).mean(std=0.5)
        return dataframe
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        #dataframe['extrema_norm'] = dataframe['extrema_norm'].fillna(0)
        ...
        #dataframe['extrema'] = dataframe['extrema'].fillna(0)
        dataframe['extrema_norm'] = dataframe['extrema'].rolling(window=5, win_type='gaussian', center=True).mean(std=0.5)        
    ...
    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        df.loc[
            (
                ...
                #(df["DI_catch"] == 1) & 
                (df["&s-extrema"] < df["&s-extrema"].shift(1)) &
                (df['maxima'].rolling(window=5).max() == 1) & 
                (df['volume'] > 0)   # Make sure Volume is not 0
        ...
        df.loc[
            (
                ...
                #(df["DI_catch"] == 1) & 
                (df["&s-extrema"] > df["&s-extrema"].shift(1)) & 
                (df['minima'].rolling(window=5).max() == 1) & 
                (df['volume'] > 0)   # Make sure Volume is not 0
